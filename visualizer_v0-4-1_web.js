...
//if(file.subtype === 'mid') {
  if(name.endsWith(".mid") || name.endsWith(".midi")){
    ...
    console.warn("Not a MIDI file:", file);
  }
}
...
function midiProcess(midiData) { 
  ...
  //check if tempo is embedded inside midi file
  if(j==0){
    isTempoEmbedded = false;
  }else{
    tempo = tempoMap[0][0];
    timeInBar = (timeInBarMidi/ppqn)*tempo/1000000;
    isTempoEmbedded = true;
  }
  //console.log(isTempoEmbedded);
...
function showBpmMsg(){
  ...
  if(isTempoEmbedded == false){
    noStroke();
    fill(209,205,183);
    rect(menuAlignX+430, 140, 300, 50);
    textFont("Helvetica");
    textSize(18);
    fill('red');
    text("Tempo not detected in MIDI file;\nplease input BPM:", menuAlignX+440, 180);
  }else{
    fill(209,205,183);
    rect(menuAlignX+430, 140, 300, 50);
  }
...
function drawThemeSel(selX, selY, selSize){
  ...
  themeSel.id("themeSelection");
  //console.log(themeSel.id());
...
function setup(){
  ...
  frameInput.id("frameInputBox");
  //console.log(frameInput.id());
  
  bpmInput = createInput("Enter BPM")
  bpmInput.position(menuAlignX+440, 200);
  bpmInput.style('font-family', 'Helvetica');
  bpmInput.style('color', 'grey');
  bpmInput.style("width", "200px");
  bpmInput.id("bpmInputBox");
  //console.log(bpmInput.id());
...
Ensure MIDI file is exported with tempo embedded (if no tempo is detected, there will be a pop-up asking for BPM input)\n
...
function draw(){
  ...
  if(startButton.value() != "True" && captureButton.value() != "True"){
    ...
    //user to input BPM if it is not embedded in MIDI file
    if(isTempoEmbedded == false){
      showBpmMsg();
      bpmInput.show();
      if(int(bpmInput.value() > 0) || isNaN(int(bpmInput.value()) == false)){  
        midiProcess(midiData);
        customBpm = true;
  ...
  if (midiData && (startButton.value() === "True"||captureButton.value() === "True")){
    ...
    if(themeSel.value()=="nier"){
      bgLines();
      drawInit(alignX, 7*fontSize+alignY, tempoMap[0][0]);
      drawProgBar();
